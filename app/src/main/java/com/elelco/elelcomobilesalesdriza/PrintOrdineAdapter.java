...
public class PrintOrdineAdapter extends Activity
        implements OnClickListener, AdapterView.OnItemClickListener, OnTouchListener, OnCheckedChangeListener, ErrorListener, StatusUpdateListener, OutputCompleteListener
{
    ...
    public void printBarcode(DettaglioFattura rigaFatt){
        int barCodeSystem = BixolonPrinter.QR_CODE_MODEL1;


        bPrinter.printText (rigaFatt.udc + "  " + rigaFatt.descrizione ,ALIGN_CENTER, bPrinter.TEXT_ATTRIBUTE_FONT_C|bPrinter.TEXT_ATTRIBUTE_EMPHASIZED,W_1, false);
    ...
    public void printReceipt(int pagenumber)// throws FileNotFoundException
    {
        ...
        try
        {
            ...
            bPrinter.printText ("    Data: " + Utility.formatDataOraRov_toData(fatt.dataOrdine),ALIGN_LEFT, FONT_A,0, false);
            DBHelper db=DBHelper.getInstance(context);
            Cliente cli=db.getCliente(fatt.codCli);
           // bPrinter.printText ("           " +  cli.ragsoc1,ALIGN_RIGHT, FONT_C|FONT_BOLD,W_1, false);
            bPrinter.printText ("  " +  String.format("%1$30s", cli.ragsoc1),ALIGN_RIGHT, FONT_C|FONT_BOLD,W_1, false);


            bPrinter.lineFeed(3,false);

            bPrinter.printText ("    |      |           Përshkrimi          |    |         |            |                |"   ,ALIGN_LEFT, FONT_B,0, false);
            bPrinter.lineFeed(1,false);
            bPrinter.printText ("    |  Nr  |             Mallit            | NM |  Sasia  |    Cmimi   |      Vlera     |"  ,ALIGN_LEFT, FONT_B,0, false);
            bPrinter.lineFeed(1,false);
            bPrinter.printText ("    |      |          ose Sherbimit        |    |         |            |                |"   ,ALIGN_LEFT, FONT_B| bPrinter.TEXT_ATTRIBUTE_UNDERLINE2,0, false);
            ...
            do{
                ...
                //nr riga
                txtRiga="    |" +String.format("%1$6s",Integer.toString(d.riga)) + "|";
                //descrizione
              //  txtRiga+=String.format("%1$-25.25s",d.descrizione)+"|";
                txtRiga+=String.format("%1$-31.31s",d.descrizione)+"|";
                //um
                if (!omaggio) {
                    txtRiga+=String.format("%1$3s",d.um)+" |";
                }else{
                    txtRiga+="    |";
                }
                //qta
                if (d.tipoRiga==Prodotto.tipologiaRigafattura.reso){
                    txtRiga+=String.format("%1$8s",Utility.formatDecimal(-d.quantita))+" |";
                }else {txtRiga+=String.format("%1$8s",Utility.formatDecimal(d.quantita))+" |";
                }//pzonunit senza iva
                txtRiga+=String.format("%1$13s",Utility.formatDecimal(Math.abs(d.prezzoSenzaIva()))+"|");
                //txtRiga+=String.format("%1$10s",Utility.formatDecimal(Math.abs(999999)).replace(",","")+"|");

                //valore senza iva
                txtRiga+=String.format("%1$17s",Utility.formatDecimal(d.valoreSenzaIva())+"|");
            ...
            if (pagenumber==totalpages-1){
                ...
                String strTot="";
                strTot=String.format("%1$30s",Utility.formatDecimal(totValoreSenzaIva));
                ...
                try
                {
                    if (fatt==null){
                     //   printReceiptVuota(1);
                        ...
                        for (DettaglioFattura rigaFatt : fatt.dett)
                        {
